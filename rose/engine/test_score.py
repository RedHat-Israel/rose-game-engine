...
class TestCoin(SinglePlayerTest):
    ...
    Coins are worthless on their own. Only collecting exactly
    `config.coins_for_jackpot` of them awards a one-time jackpot bonus;
    coins collected beyond that award no further bonus.
    """

    obstacle = obstacles.COIN

    def pickup(self):
        # track.clear() consumes the coin each tick, so put a fresh one
        # back down before every pickup to simulate collecting one per tick.
        self.track.set(self.x, self.y, self.obstacle)
        self.player.action = actions.PICKUP
        self.process()

    def test_pickup_below_jackpot(self):
        self.pickup()
        assert self.player.coins == 1
        self.assert_score(self.score)
        self.assert_remove_obstacle()

    def test_pickup_reaches_jackpot(self):
        for _ in range(config.coins_for_jackpot):
            self.pickup()
        assert self.player.coins == config.coins_for_jackpot
        expected = (
            config.coins_for_jackpot * config.score_move_forward
            + config.score_jackpot
        )
        assert self.player.score == self.score + expected

    def test_pickup_beyond_jackpot_gets_no_extra_bonus(self):
        for _ in range(config.coins_for_jackpot + 2):
            self.pickup()
        assert self.player.coins == config.coins_for_jackpot + 2
        expected = (
            config.coins_for_jackpot + 2
        ) * config.score_move_forward + config.score_jackpot
        assert self.player.score == self.score + expected
